# -*- coding: utf-8 -*-
"""
Created on Mon Jul 21 14:37:35 2025
...
# Charger ton modèle entraîné SALUTATION (chemin vers dossier où il est sauvegardé)
nlp_salutation = spacy.load("./mon_modele_salutation")

### Fonction pour détecter la salutation
def detect_salutation(text):
    doc = nlp_salutation(text)
    cats = doc.cats
    return cats.get("SALUTATION", 0) > 0.7
...
async def handle_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # CORRECTION ici pour activer la détection salutation
    if detect_salutation(question):
...
async def handle_direct_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Détection salutation ajoutée ici aussi
    if detect_salutation(question):
        await update.message.reply_text("👋 Bonjour ! Comment puis-je vous aider ?")
        return
