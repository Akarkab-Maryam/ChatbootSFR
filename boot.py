...
# Lancer la boucle interactive dans un thread séparé (non bloquant)
######threading.Thread(target=faq_sfr_complet.main, daemon=True).start()
...
async def handle_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "faq" in top_label:
        ...
        for item in faq_data["faq_sfr_complet"]:
            faq_question = item["question"].strip().lower()
